...
function SeeCommunityHouses() {
    ...
    const handleSubmit = (e) => {
        ...
        housesService.addHouse(name, country, city, continent, price)
            .then(() => {
                getAllcomunityHouse(); // Refresh the list after adding a new house
                setFormData({ name: '', city: '', country: '', price: '', continent: '' }); // Reset form fields
            })
            .catch((error) => {
    ...
    return (
        <> 
            <div className="SeeCommunityHouses-container">
                {comunityHouse.map((m) => (
